...
if($client){
    ...
    foreach ($client as $item) {
        ...
        $nfse = (new Nfse())->findByLastInvoiceId($item->id);
        ...
        if(!empty($nfse)){
            ...
            foreach ($nfse as $nf) {
                ...
                if($nf->status != 'cancelada' && date_fmt_back_month($nf->send_at) != date('m')
                    && $item->invoice_day == $day && $item->generate_invoice == 'ativo' && $item->status == 'ativo'
            ...
            if($item->invoice_day == $day && $item->generate_invoice == 'ativo' && $item->status == 'ativo' && $item->contract_duration > date('Y-m-d')){
